...
| 17 | None: supporting analysis added after the decision | `src/backtest.py`: for every FBS-vs-FBS game from 2021 to 2025, build the matchup edge from that season's earlier games only, then compare it with what the offense actually did in that game against the frozen baseline. Reported raw vs. shrunk, split by how many games each team had played, and tested against the closing point spread. A strictly prospective variant scores 2025 with the 2021–2024 model. | The project needed a falsifiable test: does this class of early-season edge predict anything? An exploratory version was run first and this entry records the design as run, before it entered the repository or any document. It is league-wide, uses only games played before each kickoff, adds no matchup finding, and changes no threshold, label, gate, or the decision. | Sept. 18 |
